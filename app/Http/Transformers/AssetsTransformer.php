...
class AssetsTransformer
{
    ...
    public function transformAsset(Asset $asset)
    {
        ...
        $carrier_details = CarrierModel::where('id',$asset->carrier_id)->get();
        $customer_name = $asset->customer_firstname.' '.$asset->customer_lastname;
        $array = [
            'id' => (int) $asset->id,
            'name' => e($asset->name),
            'asset_tag' => e($asset->asset_tag),
            'customer_name' => e($customer_name),
            'customer_address' => ($asset->customer_address) ? e($asset->customer_address) : null,
            'customer_phoneno' => e($asset->customer_phoneno),
            'model' => ($asset->model) ? [
                'id' => (int) $asset->model->id,
                'name'=> e($asset->model->name)
            ] : null,
            'model_number' => (($asset->model) && ($asset->model->model_number)) ? e($asset->model->model_number) : null,
            'barcode_number' => e($asset->barcode_number),
            ...
            'carrier' => ($asset->carrier) ? [
                'id' => (int) $asset->carrier->id,
                'name'=> e($asset->carrier->carrier_name)
            ]  : null,
            ...
            'statuslabel_list' => $this->getStatuslabelList($asset),
        ];
    ...
    public function transformRequestedAsset(Asset $asset) {
        $array = [
            ...
            'barcode_number' => ($asset->barcode_number) ? e($asset->barcode_number) : null,
            'model_number' => (($asset->model) && ($asset->model->model_number)) ? e($asset->model->model_number) : null,
            'expected_checkin' => Helper::getFormattedDateObject($asset->expected_checkin, 'date'),
    ...
    public function getStatuslabelList($asset)
    {  
        $select = '<select name="status_id_'.$asset->id.'" class="select2 status_id" onChange ="updateStatus('.$asset->id.')">';
        foreach (Helper::statusLabelListByPosition() as $format => $label) {
            $select .= '<option value="'.$format.'"'.($asset->status_id == $format ? ' selected="selected"' : '').($asset->status_id >= $format ? ' disabled' : '').'>'.$label.'</option> '."\n"; 
        } 
        $select .= '</select>'; 
        return $select; 
    }
