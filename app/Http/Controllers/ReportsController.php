...
class ReportsController extends Controller
{
    ...
    public function postCustom(Request $request)
    {
        ...
        $response = new StreamedResponse(function () use ($customfields, $request) {
            ...
            if ($request->filled('barcode_number')) {
                $header[] = trans('admin/hardware/table.barcode_number');
            }

            if ($request->filled('customer_address')) {
                $header[] = trans('admin/hardware/table.customer_address');
            }
            ...
            $assets->orderBy('assets.id', 'ASC')->chunk(20, function($assets) use($handle, $customfields, $request) {
                ...
                foreach ($assets as $asset) {
                    ...
                    if ($request->filled('barcode_number')) {
                        $row[] = ($asset->barcode_number) ? $asset->barcode_number : '';
                    }

                    if ($request->filled('customer_address')) {
                        $row[] = ($asset->customer_address) ? $asset->customer_address : '';
                    }
    ...
    */
    public function getHistoryReport()
    {
        ...
        $sql_1 = "SELECT `action_logs`.*
                , `assets`.`name` as `product_name`, `assets`.`barcode_number`
                , `users`.`first_name`, `users`.`last_name`
                , `status_labels`.`name` as `label_name`
                ...
                inner join `status_labels` on `action_logs`.`status_id` = `status_labels`.`id` 
                left join suppliers on assets.supplier_id = suppliers.id
                where DATE(action_date) = ?
                order by assets.barcode_number";
        $params = array($current_date);
        DB::enableQueryLog();
        $data['action_logs'] = DB::select($sql_1,$params);
        $sql_2 = "SELECT assets.barcode_number from assets where assets.deleted_at is NULL order by assets.barcode_number";
        $data['shipment_numbers'] = DB::select($sql_2);
        $data['vendors'] = Supplier::get();
        return view('reports/history',$data);
    }

    /**
     * Show the history report according to filter
     *
     * @author prabinthapamagar
     * @since [v5.0.7]
     * @return \Illuminate\Http\Response
    ...
    {
        $action_date_from = $request->get('action_date_from');
        $action_date_to = $request->get('action_date_to');
        $vendor_id = $request->get('vendor_id');
        $shipment_number = $request->get('shipment_number');
        $sql_1 = null;
        if(!empty($action_date_from) && !empty($action_date_to)){
            $sql_1 .= " AND DATE(action_date) between '$action_date_from' and '$action_date_to'";
        }
        else{
            if(!empty($action_date_from) && empty($action_date_to)){
                $sql_1 .= " AND DATE(action_date) >= '$action_date_from'";
            }
            else{
                if(empty($action_date_from) && !empty($action_date_to)){
                    $sql_1 .= " AND DATE(action_date) <= '$action_date_to'";
                }
            }
        }
        if(!empty($vendor_id)){
            $sql_1 .= " AND assets.supplier_id = $vendor_id";
        }
        if(!empty($shipment_number)){
            $sql_1 .= " AND assets.barcode_number = '$shipment_number'";
        }
        $sql_2 = "SELECT `action_logs`.*
                , `assets`.`name` as `product_name`, `assets`.`barcode_number`
                , `users`.`first_name`, `users`.`last_name`
                , `status_labels`.`name` as `label_name`
                ,suppliers.name as vendor_name
        ...
        if(!empty($action_logs)){
            ...
            foreach($action_logs as $action_log){
                $tr_html .= '<tr>';
                $tr_html .= '<td>'.$i.'</td>';
                $tr_html .= '<td>'.$action_log->barcode_number.'</td>';
                $tr_html .= '<td>'.$action_log->product_name.'</td>';
                $tr_html .= '<td>'.$action_log->vendor_name.'</td>';
                $tr_html .= '<td>'.$action_log->first_name.'&nbsp;'.$action_log->last_name.'</td>';
                $tr_html .= '<td>'.$action_log->action_type.'</td>';
                $tr_html .= '<td>'.date('Y-m-d H:i:s',strtotime($action_log->action_date)).'</td>';
                $tr_html .= '<td>'.$action_log->note.'</td>';
                $tr_html .= '<td>'.$action_log->label_name.'</td>';
                $tr_html .= '</tr>';
                $i++;
            }
        }
            ...
            $tr_html .= '<tr>';
            $tr_html .= '<td colspan="9" style="text-align:center;">No records found</td>';
            $tr_html .= '</tr>';
        }
        $json = array(
            'status' => 'success',
