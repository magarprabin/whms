...
class SettingsController extends Controller
{
    ...
    public function postSaveFirstAdmin(SetupUserRequest $request)
    {
        ...
        $settings->next_auto_inventory_tag_base      = 1;
        $settings->auto_increment_inventory          = $request->input('auto_increment_inventory', '0');
        $settings->auto_increment_inventory_prefix   = $request->input('auto_increment_inventory_prefix');
    ...
    public function getInventoryTags()
    {
        ...
        return view('settings.inventory_tags', compact('setting'));
    }
     ...
     * @return View
     */
    public function postInventoryTags(Request $request)
    {
        if (is_null($setting = Setting::getSettings())) {
            return redirect()->to('admin')->with('error', trans('admin/settings/message.update.error'));
        }

        $setting->auto_increment_inventory_prefix   = $request->input('auto_increment_inventory_prefix');
        $setting->auto_increment_inventory          = $request->input('auto_increment_inventory', '0');
        $setting->inventory_zerofill_count          = $request->input('inventory_zerofill_count');
        $setting->next_auto_inventory_tag_base      = $request->input('next_auto_inventory_tag_base');
