...
class CategoriesController extends Controller
{
    ...
    public function store(ImageUploadRequest $request)
    {
        ...
        if ($category->save()) {
            $categories['ref_cat_id'] = $category->id;
            $ApiRequest = new ApiRequest();        
            $result = $ApiRequest->apiCall('POST', 'categories', $categories, array());
            
            if(isset($result)) {
                $category1 = new Category();
                $category1 = Category::find($category->id);
                $category1->ref_id = $result['category_id'];
                $category1->save();
            }
    ...
    public function update(ImageUploadRequest $request, $categoryId = null)
    {
        ...
        if ($category->save()) {
            $ApiRequest = new ApiRequest();
            $ref_id = isset($category->ref_id)?$category->ref_id:$categoryId;    
            $ApiRequest->apiCall('PUT', 'categories/'.$ref_id, $categories, array());
