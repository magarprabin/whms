    ...
    public function index(Request $request, $audit = null) 
    {
        ...
        $assets = Company::scopeCompanyables(Asset::select('assets.*'),"company_id","assets")
            ->with('location', 'assetstatus', 'company', 'defaultLoc','assignedTo',
                'model.category', 'model.manufacturer', 'model.fieldset','supplier','carrier'); //it might be tempting to add 'assetlog' here, but don't. It blows up update-heavy users.
...
//        }
    ...
    public function updateFromCsCart(Request $request, $shipment_number)
    {
        ...
        $assetId = Asset::where('shipment_number', $shipment_number)->exists() ? $shipment_details[0]->id : null;
        ...
        $asset->status_id = 3;
        if (request('delivered_to_vendor')) {
            $asset->status_id = 11;
        }
        $asset->physical     = '1';
    ...
    public function addFulfillmentStarted($shipment_number,Request $request)
    {
        ...
        $asset->barcode_number = $barcode_number;
        $asset->item_id = $item_id;
        $asset->product_id = $product_id;
        $asset->product_code = $product_code;
        $asset->name = $product_name;
        $asset->customer_firstname = $customer_firstname;
        $asset->customer_lastname = $customer_lastname;
        $asset->customer_phoneno = $customer_phoneno;
        $asset->customer_address = $customer_address;
        $asset->category_name = $category_name;
        $asset->company_id = 1;
        $asset->model_id = 1;
        $asset->notes = 'Fulfillment Started';
        $asset->user_id = '1';
        $asset->archived = '0';
        $asset->physical = '1';
        $asset->depreciate = '0';
        ...
        if($trans_success == 'Y'){
            $json = array(
                'status' => 'success',
                'message' => 'Fulfillment Created successfully in warehouse!',
            );
        }
        else{
            $json = array(
                'status' => 'error',
                'message' => 'Fulfillment cound not be Created in warehouse!',
            );
        }
        return response()->json($json);
    }
