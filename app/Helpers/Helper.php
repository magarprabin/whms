...
class Helper
{
    ...
    */
     ...
     * @return Array
     */
    public static function deployableStatusLabelListDelivered()
    {
        $statuslabel_list =  Statuslabel::where('deployable', '=', '1')->where('id', '!=' ,'3')->orderBy('default_label', 'desc')
                ->orderBy('name','asc')
                ->orderBy('deployable','desc')
                ->pluck('name', 'id')->toArray();
        return $statuslabel_list;
     ...
     * @return Array
     */
    public static function deployableStatusLabelListDeliveredIndvCheckout()
    {
        $statuslabel_list =  Statuslabel::where('deployable', '=', '1')->whereNotIn('id', [3,5,11])->orderBy('default_label', 'desc')
                ->orderBy('name','asc')
                ->orderBy('deployable','desc')
                ->pluck('name', 'id')->toArray();
        return $statuslabel_list;
    ...
    /**
     * Get the list of deployable status labels in an array in Check In Form
     *
     * @todo This should probably be a selectlist, same as the other endpoints
     * and we should probably add to the API controllers to make sure that
     * the status_id submitted is actually really deployable.
     ...
     * @return Array
     */
    public static function deployableStatusLabelListCheckin()
    {
        $statuslabel_list =  Statuslabel::where('deployable', '=', '1')->whereNotIn('id', [3,6,7,9])->orderBy('default_label', 'desc')
                ->orderBy('name','asc')
                ->orderBy('deployable','desc')
                ->pluck('name', 'id')->toArray();
        return $statuslabel_list;
    ...
    /**
     * Get the list of status labels position wise in an array to make a dropdown menu
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v2.5]
     * @return Array
     */
    public static function statusLabelListByPosition()
    {
        $statuslabel_list = array('' => trans('general.select_statuslabel')) + Statuslabel::orderBy('position','asc')
                ->pluck('name', 'id')->toArray();
        return $statuslabel_list;
    }

    /**
     * Check the status level to halt backward status flow
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v2.5]
     * @return true or false
     */
    public static function checkStatusLabelByPosition($old_status_id,$new_status_id)
    {
        
        if(Statuslabel::select('position')->where('id',$old_status_id)->first() < Statuslabel::select('position')->where('id',$new_status_id)->first())
        {
            return true;
        }
        else{
