...
class Accessory extends SnipeModel
{
     ...
     * @var array
     */
    protected $searchableAttributes = ['name', 'model_number', 'order_number', 'purchase_date', 'notes', 'inventory_tag'];
...
//        'purchase_cost'     => 'numeric|nullable',
        ...
        'inventory_tag'     => 'required|min:1|max:255|unique_undeleted',
    );
    ...
    */
    ...
    {
        return $this->belongsToMany('\App\Models\User', 'accessories_users', 'accessory_id', 'assigned_to')->withPivot('id', 'created_at', 'note', 'order_number', 'accessory_id')->withTrashed();
    }
    ...
    */
     ...
     * @return int
     */
    public static function nextInventoryAutoIncrement($inventories)
    {
    ...
    /**
     * Get the next autoincremented inventory tag
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v4.0]
     * @return string | false
     */
    public static function autoincrement_inventory()
    {
        $settings = \App\Models\Setting::getSettings();
        if ($settings->auto_increment_inventory == '1') {
            $temp_inventory_tag = \DB::table('accessories')
                ->where('physical', '=', '1')
                ->max('inventory_tag');

            $inventory_tag_digits = preg_replace('/\D/', '', $temp_inventory_tag);
            $inventoryt_tag = preg_replace('/^0*/', '', $inventory_tag_digits);
            $last_tag_base = $settings->next_auto_inventory_tag_base;
            $new_tag_base = $last_tag_base + 1;
            if ($settings->inventory_zerofill_count > 0) {
                // return $settings->auto_increment_inventory_prefix.Accessory::inventoryzerofill($settings->next_auto_inventory_tag_base, $settings->inventory_zerofill_count);
                return $settings->auto_increment_inventory_prefix.Accessory::inventoryzerofill($new_tag_base, $settings->inventory_zerofill_count);
            }
            return $settings->auto_increment_inventory_prefix.$settings->next_auto_inventory_tag_base;
        } else {
            return false;
        }
    }
    ...
    /**
     * Add inventory zerofilling based on Settings
     *
     * We'll add the zerofill and prefixes on the fly as we generate the number.
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     ...
     * @return string
     */
    public static function inventoryzerofill($num, $zerofill = 3)
    {
