...
<div class="form-group{{ $errors->has('statuslabel_mappings') ? ' has-error' : '' }}">
    <label for="statuslabel_mappings" class="col-md-3 control-label">{{ trans('admin/statuslabels/table.statuslabel_mappings') }} </label>
    <div class="col-md-7 required">
        {{ Form::select('statuslabel_mappings', $statuslabel_mappings, $item->getMappedStatus($item), array('class'=>'select2', 'style'=>'min-width:400px', 'aria-label'=>'statuslabel_mappings')) }}
        {!! $errors->first('statuslabel_mappings', '<span class="alert-msg" aria-hidden="true"><i class="fa fa-times" aria-hidden="true"></i> :message</span>') !!}
    </div>
</div>
...
<div class="form-group {{ $errors->has('position') ? ' has-error' : '' }}">
    <label for="name" class="col-md-3 control-label">{{ trans('admin/statuslabels/table.position') }}</label>
    <div class="col-md-3 required">
        <input class="form-control" type="number" name="position" aria-label="name" id="name" value="{{ old('position', $item->position) ? old('position', $item->position) : (isset($max_position) ? $max_position + 1:0) }}" data-validation="required" />
        {!! $errors->first('position', '<span class="alert-msg" aria-hidden="true">:message</span>') !!}
    </div>
</div>
