...
@push('js')
    <script src="{{ url('js/dist/bootstrap-table-assets.js') }}" nonce="{{ csrf_token() }}"></script>
    ...
    <script nonce="{{ csrf_token() }}">
        ...
        function ridersActionFormatter(value, row) {
            var actions = '<nobr>';
            var routes = 'riders';
            if ((row.available_actions) && (row.available_actions.update === true)) {
                actions += '<a href="{{ url('/') }}/' + routes + '/' + row.id + '/edit" class="btn btn-sm btn-warning" data-tooltip="true" title="{{ trans('general.update') }}"><i class="fa fa-pencil" aria-hidden="true"></i><span class="sr-only">{{ trans('general.update') }}</span></a>&nbsp;';
            }

            // if ((row.available_actions) && (row.available_actions.delete === true)) {
            actions += '<a href="{{ url('/') }}/' + routes + '/' + row.id + '" '
        ...
        function podActionFormatter(value,row) {
            var actions = '<nobr>';
            var routes = 'pod';
            if ((row.available_actions) && (row.available_actions.update === true)) {
                actions += '<a href="{{ url('/') }}/' + routes + '/' + row.id + '/edit" class="btn btn-sm btn-warning" data-tooltip="true" title="{{ trans('general.update') }}"><i class="fa fa-pencil" aria-hidden="true"></i><span class="sr-only">{{ trans('general.update') }}</span></a>&nbsp;';
                actions += '<a href="{{ url('/') }}/' + routes + '/' + row.id + '/print" class="btn btn-sm btn-primary" data-tooltip="true" title="{{ trans('admin/pod/general.print') }}">{{ trans('admin/pod/general.print') }}</a>&nbsp;';
                actions += '<a href="{{ url('/') }}/' + routes + '/' + row.id + '/checkin" class="btn btn-sm btn-success" data-tooltip="true" title="{{ trans('general.checkin') }}">{{ trans('general.checkin') }}</a>&nbsp;';
                actions += '<a href="{{ url('/') }}/' + routes + '/' + row.id + '/" class="btn btn-sm btn-warning" data-tooltip="true" title="{{ trans('general.view_all') }}"><i class="fa fa-eye" aria-hidden="true"></i><span class="sr-only">{{ trans('general.view_all') }}</span></a>&nbsp;';
                actions += '<a href="{{ url('/') }}/' + routes + '/' + row.id + '/upload" class="btn btn-sm bg-purple" data-tooltip="true" title="{{ trans('admin/pod/general.upload') }}">{{ trans('admin/pod/general.upload') }}</a>&nbsp;';
            }
            return actions;
        }
        ...
        function genericActionsFormatter(owner_name, element_name) {
            ...
            return function (value,row) {
                ...
                var status_id = row.status_label.id;
                if ((row.available_actions) && (row.available_actions.update === true) && (status_id == 7 || status_id == 10)) {
        ...
        function genericCheckinCheckoutFormatter(destination) {
            return function (value,row) {
                ...
                if ((row.available_actions.checkout == true) && (row.user_can_checkout == true) && ((!row.asset_id) && (!row.assigned_to))) {
                    if(row.asset_tag == ''){
                        var checkout_disabled = '';
                    }
                    else{
                        var checkout_disabled = 'disabled';
                    }
                    return '<a href="{{ url('/') }}/' + destination + '/' + row.id + '/checkout" class="btn btn-sm bg-maroon '+checkout_disabled+'" data-toggle="tooltip" title="Check this item out">{{ trans('general.checkout') }}</a>';
        ...
        function genericLabelFormatter(destination) {
            return function (value,row) {

                // The user is allowed to check items out, AND the item is deployable
                if(row.asset_tag != ''){
                    return '<a href="{{ url('/') }}/' + destination + '/' + row.id + '/printlabel" class="btn btn-sm bg-green btn-label-print" data-toggle="tooltip" title="Print Label" target="_blank">Print Tag</a>';
                }
            }
        }

        function genericLabelgenFormatter(destination) {
            return function (value,row) {

                // The user is allowed to check items out, AND the item is deployable
                return '<a href="{{ url('/') }}/' + destination + '/' + row.id + '/generateqr" class="btn btn-sm bg-green btn-label-generate" data-toggle="tooltip" title="Generate Label">Generate</a>';
        ...
        for (var i in formatters) {
            ...
            window[formatters[i] + 'LabelFormatter'] = genericLabelFormatter(formatters[i]);
            window[formatters[i] + 'LabelgenFormatter'] = genericLabelgenFormatter(formatters[i]);
        }
        ...
        function deployedCarrierFormatter(row, value) {
            if ((row) && (row!=undefined)) {
                return '<a href="{{ url('/') }}/carrier/' + row.id + '">' + row.name + '</a>';
            }
        }

        function carrierFormatter(row, value) {
            if(value.carrier_id){
                return value.carrier_id.name;
            }
        }
