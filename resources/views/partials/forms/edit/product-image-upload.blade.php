<div class="form-group {{ $errors->has((isset($fieldname) ? $fieldname : 'image')) ? 'has-error' : '' }}">
    ...
    <div class="col-md-9">
        ...
        <label class="btn btn-default" aria-hidden="true">
            ...
            <input type="file" name="{{ (isset($fieldname) ? $fieldname : 'image1') }}" class="js-uploadFile" id="uploadFile" data-maxsize="{{ \App\Helpers\Helper::file_upload_max_size() }}" accept="image/gif,image/jpeg,image/webp,image/png,image/svg,image/svg+xml" style="display:none; max-width: 90%" aria-label="image" aria-hidden="true">
        </label>
