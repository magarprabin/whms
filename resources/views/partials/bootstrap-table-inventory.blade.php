...
<script nonce="{{ csrf_token() }}">
    ...
    function genericActionsFormatter(owner_name, element_name) {
        ...
        return function (value,row) {
            ...
            if ((row.available_actions) && (row.available_actions.update === true) && (row.user_can_checkout == true)) {
    ...
    function genericLabelFormatter(destination) {
        return function (value,row) {
            ...
            // The user is allowed to check items out, AND the item is deployable
            return '<a href="{{ url('/') }}/' + destination + '/' + row.id + '/printlabel" class="btn btn-sm bg-purple" data-toggle="tooltip" title="Print Label" target="_blank">Print</a>';
