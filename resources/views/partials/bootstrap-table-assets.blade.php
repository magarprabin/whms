...
<script nonce="{{ csrf_token() }}">
    ...
    // These methods dynamically add/remove hidden input values in the bulk status actions form
    $('.snipe-table').on('check.bs.table .btSelectItem', function (row, $element) {
        $('#bulkStatusEdit').removeAttr('disabled');
        // $('#bulkStatusEdit').prepend('<input id="checkbox_' + $element.id + '" type="hidden" name="ids[]" value="' + $element.id + '">');
    });
    ...
    $('.snipe-table').on('uncheck.bs.table', function () {
        if ($('.snipe-table').bootstrapTable('getSelections').length == 0) {
            $('#bulkStatusEdit').attr('disabled', 'disabled');
        }
    });
    ...
    $('.snipe-table').on('check-all.bs.table', function (event, rowsAfter, rowsBefore) {
        ...
        for (var i in rowsAfter) {
            // console.log(rowsAfter[i].id);
            // $('#bulkStatusEdit').prepend('<input id="checkbox_' + rowsAfter[i].id + '" type="hidden" name="ids[]" value="' + rowsAfter[i].id + '">');
        }
    });
   ...
   function imageFormatter(value, row) {
    ...
    $(function () {
        $('#bulkStatusEdit').click(function () {
            var selectedIds = $('.snipe-table').bootstrapTable('getSelections');
            $.each(selectedIds, function(key,value) {
                $( "#bulkStatusForm" ).append($('<input type="hidden" name="ids[' + value.id + ']" value="' + value.id + '">' ));
            });
    ...
    function statuslabelsEditFormatter(value, row) {
        return row.statuslabel_list;       
    }

    function updateStatus(asset_id){

        var status_id  = $("select[name='status_id_"+asset_id+"'] option:selected").val();
        if(status_id !== '') {
            $.ajax({
                type:'POST',
                url: "{{url('/') }}/hardware/statusupdate",
                data:{'ids':[asset_id],'status_id':status_id},
                headers: {
                ...
                success:function(response){
                    if(response.status == 'success'){
                        window.location = '{{route("hardware.index")}}';
                    }
