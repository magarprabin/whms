...
@section('inputFields')
  ...
  <div class="form-group {{ $errors->has('asset_tag') ? ' has-error' : '' }}">
    <label for="asset_tag" class="col-md-3 control-label">Order Shipment Barcode Number</label>
    <input class="form-control" type="hidden" name="asset_tags[1]" id="asset_tag" value="{{ Request::old('asset_tag', $item->asset_tag) }}" data-validation="required">
      <!-- we are editing an existing asset -->
      <div class="col-md-7 col-sm-12">
          <input class="form-control" type="text" readonly name="barcode_number[1]" id="barcode_number" value="{{ Request::old('barcode_number', $item->barcode_number) }}">
          </div>
  </div>
  ...
  {{-- <div class="form-group" style="margin-top: 10px;">
    <label for="status_id" class="col-md-3 control-label">Changes to Status</label>
    <div class="col-md-7 col-sm-11">
        @if($item->status_id == '10')
        <input type="hidden" name="status_id" value="11">
        <input type="text" name="" class="form-control" value="Delivered to Vendor" readonly>
        @else
        <input type="hidden" name="status_id" value="3">
        <input type="text" name="" class="form-control" value="Delivered" readonly>
        @endif
    </div>
...
@include ('partials.forms.edit.status_assets')
  @if (!$item->id)
  ...
  @elseif (($item->assignedTo) && ($item->deleted_at==''))
      ...
      @include ('partials.forms.edit.datepicker-asset-checkin', ['translated_name' => trans('admin/hardware/form.expected_checkin'),'fieldname' => 'expected_checkin'])
  @endif

  @include ('partials.forms.edit.name', ['translated_name' => trans('admin/hardware/form.name')])
  <?php /*@include ('partials.forms.edit.purchase_date')*/?>
  @include ('partials.forms.edit.supplier-select-assets', ['translated_name' => trans('general.supplier'), 'fieldname' => 'supplier_id'])
  ...
  <input type="hidden" value="0" name="requestable" id="requestable" class="minimal">
...
<script nonce="{{ csrf_token() }}">
    ...
    $(document).ready(function() {
        ...
        $("#status_select_id").change(function( event ) {
            var status = $(this).val();
            if(status == 7){
        ...
        //disable all option value before selected option (select option list already sorted according to position)
        var array = [];
        $("#status_select_id option").each(function(n)
        {
            array[n] = $(this).val();
        });

        var selected_val = parseInt($('#status_select_id').find(":selected").val());
        $.each(array, function(key, val) {

            // if(val <= selected_val){
            if(!$('select[name=status_id] option[value="'+val+'"]').is(':selected')){
                $('select[name=status_id] option[value="'+val+'"]').prop('disabled', true);
            }else{
                return false;
            }
